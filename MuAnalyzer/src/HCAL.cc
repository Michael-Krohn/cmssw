...
double* HCAL::HitsPlots(const edm::Event& iEvent, const edm::EventSetup& iSetup, edm::EDGetTokenT<edm::SortedCollection<HBHERecHit,edm::StrictWeakOrdering<HBHERecHit> >> HBHERecHit_Label, double MuonEta, double MuonPhi, double ConeSize, Histograms myHistograms){ //TH1F* Spectra[7], TH2F* layer_eta[7], TH1F* missinghits, TH1F* missinghitseta){
  ...
  else
  {
    ...
    {   
	 ...
	 myHistograms.m_Layer_Eta[id.depth()-1]->Fill(hbherechit->energy(),hbhe_position.eta());
         ...
         MuonHits[id.depth()-1].push_back(std::make_tuple(hbhe_position.eta(),hbhe_position.phi(),hbherechit->energy()));
       }
