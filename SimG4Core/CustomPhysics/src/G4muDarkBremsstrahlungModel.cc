...
G4muDarkBremsstrahlungModel::G4muDarkBremsstrahlungModel(const G4ParticleDefinition* p,
   ...
   : G4VEmModel(nam),
     ...
     lhe_loaded(false)
{
   if(p) { SetParticle(p); } //Verify that the particle is a muon.
...
void G4muDarkBremsstrahlungModel::ParseLHE (std::string fname)
   ...
   while(std::getline(ifile,line))
   {
         ...
         if((ptype==11)&&(state==-1))
	 {
	    ...
	    jss >> ptype >> state >> skip >> skip >> skip >> skip >> e_px >> e_py >> e_pz >> e_E >> e_M; 
            if((ptype==11)&&(state==1)) //Find a final state muon.
            {
               for(int i=0;i<2;i++) {std::getline(ifile,line);}
...
// Calculates the cross section per atom in GEANT4 internal units. Uses WW approximation to find the total cross section, performing numerical integrals over x and theta.
{
   ...
   //printf("Biased the cross section.\n");
   if(cross < 0.) 
   { 
      cross = 0.; 
   }
...
/*   G4ProcessVector* pVec = ptable->FindProcesses();
   for(int i=0; i<pVec->size();i++)
   {
      G4VProcess* pro = (*pVec)[i];
      std:: cout << pro->GetProcessName() << std::endl;
   }*/
   ...
   if(lhe_loaded==false)
   {
      ...
      DIR *dir;
      dir = opendir("/hdfs/cms/user/revering/madgraph/muon/"); 
   ...
   G4double momentum = sqrt(EAcc*EAcc-muon_mass_mev*muon_mass_mev); //Electron momentum in MeV.
   ...
   if(makeSecondary) {
     ...
     else
     {
        G4DynamicParticle* mu = 
          new G4DynamicParticle(const_cast<G4ParticleDefinition*>(particle),
                                dp->GetMomentumDirection(), dp->GetTotalEnergy());
        vdp->push_back(mu);
     }
     // continue tracking
   } else {
   ...
   //fParticleChange->SetProposedMomentumDirection(dp->GetMomentum().unit());
   //fParticleChange->SetProposedKineticEnergy(dp->GetTotalEnergy()-muon_mass_mev);
   }
 } 
